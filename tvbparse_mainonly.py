...
def extractInfobox(x):
    ...
    for line in infobox_alllines:
        if line.find("th") is not None:
            ...
            if line.find("th").get_text() == "演出".decode('utf-8'):
                stars = line.find('td').get_text()
    return {"genre": genre, "noepi": noepi, "direct": direct, "stars": stars}
...
for drama in cleandrama:
    try:
        alldata.append(crawldramax(drama))
    except:
        print "Failed"
        failed.append(drama)
...
csvfilename = '/home/chanch/tvbrank/allstars.csv'

ofile = open(os.path.abspath(csvfilename), 'wb')
writer = csv.writer(ofile)

for drama in alldata:
    writer.writerow([drama['info']['stars'].encode('utf-8'), drama['name'].encode('utf-8'), drama['year'], drama['type'], drama['info']['genre'].encode('utf-8'), drama['info']['direct'].encode('utf-8'), drama['info']['noepi'].encode('utf-8')])
ofile.close()
