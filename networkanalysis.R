impdata <- cleandata[cleandata$V1 %in% names(table(cleandata$V1)[table(cleandata$V1) >= 4]),]
...
sapply(uniprod, getEvcProd, evcproducers = evcproducers)
...
png("testgraph.png", width = 3000, height = 2000)
plot(MVPonly, vertex.size = V(MVPonly)$evc * 10, vertex.label.cex = 3, vertex.color = "white", edge.width = (E(MVPonly)$weight / max(E(MVPonly)$weight)) * 3,  layout = layout.fruchterman.reingold, vertex.label.color = "black")
dev.off()
